...
cells.append(code(
    ...
    "# 10 matches the literature config (arXiv 2508.18984: rerank pool 20 -> 10).\n"
    "GEN_TOP_N = 10\n"
    "# Page-aware context: feed the FULL text of the top-ranked page(s) instead\n"
    "# of N scattered nodes. Was the best config under the BI-encoder (0.593),\n"
    "# but discards the chunk-level precision a cross-encoder provides -- with\n"
    "# the cross-encoder ON, the literature config is top-N chunks (False).\n"
    "PAGE_CONTEXT       = True   # whole-page context: our OCR elements are tiny\n"
    "                            # line fragments; top-N of them starves the LLM\n"
    "                            # (ANLS 0.25 vs 0.6 page mode). Cross-encoder\n"
    "                            # still drives the RANKING that picks the page.\n"
    "PAGE_CONTEXT_N     = 2      # top-1 + top-2 ranked pages\n"
    "PAGE_CONTEXT_CHARS = 8000   # build_prompt truncation in page mode\n"
    "# Page selection mode (page mode only): 'first_seen' empirically best.\n"
    "PAGE_SCORE_MODE    = 'first_seen'  # 'first_seen' | 'mean_topk' | 'sum'\n"
    "PAGE_SCORE_TOPK    = 3             # k for mean_topk\n"
    "# Cross-encoder reranker: two-stage retrieval precision stage. Jointly\n"
    "# scores (question, chunk); crushes close-but-wrong date/label chunks the\n"
    "# bi-encoder ranks high (verified: answer +6.8 vs date noise -10.5).\n"
    ...
    "# True HYBRID fusion: on HYBRID-routed questions, GraMM fuses the graph/\n"
    "# self-healing candidate set with FAISS top-k (union, deduped), THEN the\n"
    "# cross-encoder reranks. Graph recall + dense recall + precision rerank --\n"
    "# the standard fusion architecture, and what the HYBRID route means.\n"
    "HYBRID_FUSION      = True\n"
    "# Graph-guided page selection: with fusion+cross-encoder, the top-ranked\n"
    "# node is almost always a shared FAISS candidate, so GraMM's page choice\n"
    "# collapses onto the baseline's (exact tie, 0.609=0.609). The graph's\n"
    "# measured edge is PAGE localisation (APPA@1 0.41 vs 0.30), so use it at\n"
    "# the page level: GraMM's 2-page context = cross-encoder best page +\n"
    "# graph-retrieval best page. Always contains the baseline's page, so it\n"
    "# can only add the gold page more often.\n"
    "GRAPH_PAGE_GUIDE   = True\n"
    "# Graph augmentation on EVERY question (not just GRAPH/HYBRID-routed).\n"
    "# The heuristic router sends 63% of questions down a pure-vector path\n"
    "# where GraMM == baseline by construction, capping any aggregate edge.\n"
    "# GraMM-RAG's design claim is graph-AUGMENTED retrieval; with fusion the\n"
    "# graph path is a superset of the vector evidence (page 1 is always the\n"
    "# baseline's page), so augmenting everywhere can tie but not lose.\n"
    "# Router decisions are still logged for the per-route analysis.\n"
    "ROUTE_ALL_HYBRID   = True\n"
    "# Self-healing GENERATION (GraMM only -- the baseline is flat RAG by\n"
    "# definition). The quote-then-answer EVIDENCE line is a free verification\n"
    "# signal: if the quoted evidence does not appear in the supplied context,\n"
    "# the extraction is hallucinated; retry ONCE with the graph's page\n"
    "# promoted to first position. Extends the dissertation's reward-based\n"
    "# self-healing from retrieval into generation.\n"
    "SELF_HEAL_GENERATION = True\n"
    "\n"
    "# Stamped into result JSONs; cached results are reused ONLY if their\n"
    "# config matches. Kills the stale-cache trap (results from an old config\n"
    "# being silently reloaded and reported as the new config's numbers).\n"
    "RUN_CONFIG = {\n"
    "    'N_PILOT': N_PILOT, 'SEED': SEED, 'GEN_TOP_N': GEN_TOP_N,\n"
    "    'PAGE_CONTEXT': PAGE_CONTEXT, 'PAGE_CONTEXT_N': PAGE_CONTEXT_N,\n"
    "    'PAGE_SCORE_MODE': PAGE_SCORE_MODE,\n"
    "    'USE_CROSS_ENCODER': USE_CROSS_ENCODER, 'RERANK_POOL': RERANK_POOL,\n"
    "    'HYBRID_FUSION': HYBRID_FUSION, 'GRAPH_PAGE_GUIDE': GRAPH_PAGE_GUIDE,\n"
    "    'ROUTE_ALL_HYBRID': ROUTE_ALL_HYBRID,\n"
    "    'PROMPT_V': 3,   # quote-then-answer (EVIDENCE/ANSWER) extraction scaffold\n"
    "    'SELF_HEAL_GENERATION': SELF_HEAL_GENERATION,\n"
    "}\n"
    "\n"
    "import sys, pathlib\n"
...
# ── Shared retrieval helpers (defined early: Phase 5 + 8 + 9 all use them) ─────
cells.append(md(
    "## Retrieval Helpers\n"
    "\n"
...
cells.append(code(
    ...
    "\n"
    "def node_page_of(nd, type_to_elems):\n"
    "    \"\"\"Page number of one node dict, or None.\"\"\"\n"
    "    elems = type_to_elems.get(nd.get('node_type'), [])\n"
    "    li = nd.get('local_idx')\n"
    "    if li is not None and 0 <= li < len(elems):\n"
    "        try:\n"
    "            return int(elems[li].get('page_no', -1))\n"
    "        except (TypeError, ValueError):\n"
    "            return None\n"
    "    return None\n"
    "\n"
    "def pages_to_nodes(page_list, parsed):\n"
    "    \"\"\"Expand an explicit ordered page list into reading-order node dicts\n"
    "    (the second half of page_context_nodes, with the page choice external).\"\"\"\n"
    "    elements = (parsed or {}).get('elements', [])\n"
    "    type_to_elems = {}\n"
    "    for e in elements:\n"
    "        type_to_elems.setdefault(e['type'], []).append(e)\n"
    "    sel = [p for p in page_list if p is not None]\n"
    "    out = []\n"
    "    for nt, elems in type_to_elems.items():\n"
    "        for li, e in enumerate(elems):\n"
    "            try:\n"
    ...
    "                except (TypeError, ValueError):\n"
    "                    ro = 0\n"
    "                out.append((sel.index(pg), ro, {'node_type': nt, 'local_idx': li}))\n"
    "    out.sort(key=lambda t: (t[0], t[1]))\n"
    "    return [nd for _, _, nd in out]\n"
    "\n"
    "def parse_answer(raw):\n"
    "    \"\"\"Extract the value from the quote-then-answer (EVIDENCE/ANSWER)\n"
    "    response format. Falls back to the raw string when absent.\"\"\"\n"
    "    if not isinstance(raw, str):\n"
    "        return raw\n"
    "    if 'ANSWER:' in raw:\n"
    "        return raw.rsplit('ANSWER:', 1)[-1].strip().splitlines()[0].strip() \\\n"
    "               if raw.rsplit('ANSWER:', 1)[-1].strip() else raw.strip()\n"
    "    return raw.strip()\n"
    "\n"
    "print('Retrieval helpers ready (embed_query, node_pages, rerank_by_raw_e5, '\n"
    "      'page_context_nodes, cross_rerank, pages_to_nodes, parse_answer).')"
))
...
cells.append(code(
    ...
    "    _cached = json.loads(vector_out_path.read_text())\n"
    "    if _cached.get('config') != RUN_CONFIG:\n"
    "        print('Cached vector results are from a DIFFERENT config -- recomputing.')\n"
    "        _cached = None\n"
    "if _cached is not None:\n"
    "    print(f'Flat-vector RAG results exist (config match): {vector_out_path}')\n"
    "    vector_results = _cached\n"
    "else:\n"
    ...
    "        answer = parse_answer(\n"
    "            vlm_vec.generate(prompt, temperature=0.0, max_tokens=128))\n"
    "\n"
...
cells.append(code(
    ...
    "\n"
    "        vi = faiss_indices.get(doc_id)\n"
    "        _use_graph = (ROUTE_ALL_HYBRID or route in ('GRAPH', 'HYBRID'))\n"
    "        if graph and parsed and _use_graph:\n"
    "            result = retriever.retrieve(q_vec, graph, parsed, [])\n"
    "            # TRUE HYBRID: the HGT/self-healing pool has great page recall\n"
    "            # (APPA@10 0.73) but the answer node is rarely rank-1, so page\n"
    "            # selection misses it -> refusals. Merge the raw-E5 FAISS\n"
    "            # candidates (the signal that makes the VECTOR path work) into\n"
    "            # the page-selection pool: graph recall + vector precision.\n"
    "            # Self-healing's reward/refused decision is left untouched.\n"
    "            pool = list(result['nodes'])\n"
    "            if HYBRID_FUSION and vi and vi.index:\n"
    ...
    "                        pool.append(n)\n"
    "        else:\n"
    "            _pool_k = RERANK_POOL if USE_CROSS_ENCODER else 10\n"
    "            nodes = vi.search(q_vec, top_k=_pool_k) if vi and vi.index else []\n"
    "            result = {'nodes': nodes, 'reward': 0.5, 'refused': False,\n"
    "                      'rounds': 0, 'reward_detail': {}}\n"
    "            pool = list(nodes)\n"
    "\n"
    "        # Rerank the (merged) pool; bi-encoder default, cross-encoder opt.\n"
    "        # APPA below still uses result['nodes'] (graph-only) so it stays a\n"
    "        # pure graph-retrieval metric, unaffected by the merge.\n"
    "        if USE_CROSS_ENCODER:\n"
    "            ranked = cross_rerank(pool, question, doc_id, parsed)\n"
    "        else:\n"
    "            ranked = rerank_by_raw_e5(pool, q_vec, doc_id, parsed)\n"
    "        if PAGE_CONTEXT:\n"
    "            _graph_nodes = result['nodes']\n"
    "            if (GRAPH_PAGE_GUIDE and _use_graph\n"
    "                    and _graph_nodes and parsed):\n"
    "                # Graph-guided page selection: page 1 = best cross-ranked\n"
    "                # node overall (same evidence the baseline uses); page 2 =\n"
    "                # the GRAPH retriever's best page (its distinctive page-\n"
    "                # localisation signal). Context therefore always contains\n"
    "                # the baseline's page and adds the graph's page.\n"
    "                _t2e = {}\n"
    "                for _e in parsed.get('elements', []):\n"
    "                    _t2e.setdefault(_e['type'], []).append(_e)\n"
    "                p1 = next((node_page_of(n, _t2e) for n in ranked\n"
    "                           if node_page_of(n, _t2e) is not None), None)\n"
    "                _gsorted = sorted(_graph_nodes,\n"
    "                                  key=lambda n: n.get('rerank_score',\n"
    "                                                      n.get('score', 0.0)),\n"
    "                                  reverse=True)\n"
    "                p2 = next((node_page_of(n, _t2e) for n in _gsorted\n"
    "                           if node_page_of(n, _t2e) not in (None, p1)), None)\n"
    "                if p2 is None:   # graph agrees on p1 -> next cross page\n"
    "                    p2 = next((node_page_of(n, _t2e) for n in ranked\n"
    "                               if node_page_of(n, _t2e) not in (None, p1)),\n"
    "                              None)\n"
    "                gen_nodes = pages_to_nodes([p1, p2], parsed)\n"
    "                _heal_pages = ([p2, p1] if (p1 is not None and p2 is not None\n"
    "                                            and p1 != p2) else None)\n"
    "                if not gen_nodes:\n"
    "                    gen_nodes = page_context_nodes(ranked, parsed,\n"
    "                                                   PAGE_CONTEXT_N)\n"
    "            else:\n"
    "                gen_nodes = page_context_nodes(ranked, parsed, PAGE_CONTEXT_N)\n"
    "                _heal_pages = None\n"
    ...
    "            benchmark='mpdocvqa', max_context_chars=mcc,\n"
    "        )\n"
    "        raw_answer = vlm_eval.generate(prompt, temperature=0.0, max_tokens=128)\n"
    "        answer = parse_answer(raw_answer)\n"
    "\n"
    "        # Self-healing GENERATION: verify the model's quoted EVIDENCE line\n"
    "        # actually occurs in the supplied context; if hallucinated, retry\n"
    "        # once with the graph page promoted to first position.\n"
    "        if (SELF_HEAL_GENERATION and _heal_pages\n"
    "                and isinstance(raw_answer, str) and 'EVIDENCE:' in raw_answer):\n"
    "            _ev = raw_answer.split('EVIDENCE:', 1)[-1]\n"
    "            _ev = _ev.split('ANSWER:', 1)[0].strip().lower()\n"
    "            _ev_toks = set(re.sub(r'[^a-z0-9 ]', ' ', _ev).split())\n"
    "            _t2e_h = {}\n"
    "            for _e in (parsed or {}).get('elements', []):\n"
    "                _t2e_h.setdefault(_e['type'], []).append(_e)\n"
    "            _ctx_txt = ' '.join(\n"
    "                _t2e_h.get(nd['node_type'], [{}])[nd['local_idx']].get('text', '')\n"
    "                for nd in gen_nodes\n"
    "                if nd.get('local_idx', -1) < len(_t2e_h.get(nd.get('node_type'), []))\n"
    "            ).lower()\n"
    "            _ctx_toks = set(re.sub(r'[^a-z0-9 ]', ' ', _ctx_txt).split())\n"
    "            _overlap = (len(_ev_toks & _ctx_toks) / len(_ev_toks)\n"
    "                        if _ev_toks else 1.0)\n"
    "            if _overlap < 0.6:   # quoted evidence not in context -> heal\n"
    "                gen2 = pages_to_nodes(_heal_pages, parsed)\n"
    "                if gen2:\n"
    "                    p2prompt = build_prompt(\n"
    "                        question, gen2, parsed or {'elements': []},\n"
    "                        result['reward_detail'], result['refused'],\n"
    "                        benchmark='mpdocvqa', max_context_chars=mcc,\n"
    "                    )\n"
    "                    a2 = parse_answer(vlm_eval.generate(\n"
    "                        p2prompt, temperature=0.0, max_tokens=128))\n"
    "                    if a2 and isinstance(a2, str) and a2.strip():\n"
    "                        answer = a2\n"
    "\n"
...
cells.append(code(
    ...
    "    if out_path.exists():\n"
    "        _c = json.loads(out_path.read_text())\n"
    "        if _c.get('config') == RUN_CONFIG:\n"
    "            print(f'Seed {seed}: results exist (config match) at {out_path}')\n"
    "            gramm_results[seed] = _c\n"
    "            continue\n"
    "        print(f'Seed {seed}: cached results from a DIFFERENT config -- recomputing.')\n"
    "    print(f'\\n=== GraMM-RAG / seed={seed} ===')\n"
...
cells.append(md(
    ...
    "pilot scale carry run-to-run noise -- read the *direction*, confirm at full\n"
    "scale with the bootstrap."
))
...
cells.append(md(
    ...
    "\n"
    "Paired per-question exact-match outcomes. `GraMM-only`/`Vec-only` are the\n"
    "discordant pairs the verdict actually rests on; McNemar's chi-square tests\n"
    "whether that imbalance is significant. At pilot N this is usually n.s. --\n"
    "the point of the table is to show *how few* questions decide it, motivating\n"
    "the full-run paired bootstrap."
))
...
cells.append(code(
    ...
    "    return any(pn == normalise_answer(g) for g in gs)\n"
    "for seed, res in sorted(gramm_results.items()):\n"
    "    P, G = res['predictions'], res['golds']\n"
    "    if not vr:\n"
...
cells.append(md(
    ...
    "\n"
    "Where each system's answers fall: `exact` (ANLS=1), `partial`\n"
    "(0<ANLS<1), `zero` (ANLS=0, non-refusal), `refusal` (model declined).\n"
    "The robust GraMM mechanism story is here -- fewer refusals, more exact --\n"
    "and it holds even when aggregate ANLS is noise-dominated."
))
...
cells.append(code(
    ...
    "    gs = gold if isinstance(gold, list) else [gold]\n"
    "    a = max(anls_score(pred, g) for g in gs) if gs else 0.0\n"
    "    if a >= 0.999:\n"
    "        return 'exact'\n"
    "    if a > 0.0:\n"
    "        return 'partial'\n"
...
cells.append(md(
    ...
    "\n"
    "Validates a core thesis contribution: does the self-healing reward signal\n"
    "track answer quality? Per-question reward binned, ANLS distribution per\n"
    "bin. A rising trend = the reward is a meaningful retrieval-quality proxy."
))
...
cells.append(code(
    ...
    "    edges = np.quantile(rw, [0, .2, .4, .6, .8, 1.0])\n"
