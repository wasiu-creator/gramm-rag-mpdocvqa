...
cells.append(code(
    ...
    "\n"
    "        # Self-healing already used the full (expanded) node set for the\n"
    "        # reward/refusal decision above; the LLM sees only the top-N so a\n"
    "        # bigger/noisier graph context can't dilute generation.\n"
    "        gen_nodes = sorted(result['nodes'],\n"
    "                           key=lambda n: n.get('score', 0.0),\n"
    "                           reverse=True)[:GEN_TOP_N]\n"
    "        prompt = build_prompt(\n"
    "            question, gen_nodes,\n"
    "            parsed or {'elements': []},\n"
