...
cells.append(code(
    ...
    "# Page selection: aggregate each page's retrieved-node scores instead of\n"
    "# 'first appearance'. 'sum' over retrieved nodes is the default: it rewards\n"
    "# the page graph expansion put many (medium) answer nodes on, and beats a\n"
    "# page with one spurious high node -- exactly GraMM's HYBRID failure mode.\n"
    "# (Sum is over RETRIEVED nodes only, so it is not dominated by page length.)\n"
    "# 'mean_topk' is the peak-relevance alternative if sum over-favours density.\n"
    "PAGE_SCORE_MODE    = 'sum'         # 'sum' | 'mean_topk'\n"
...
cells.append(code(
    ...
    "    # Rank by aggregate desc; stable on first-seen order for ties.\n"
    ...
    "      'rerank_by_raw_e5, page_context_nodes).')"
))
...
cells.append(code(
    ...
    "\n"
    "        # Page-aware context (or legacy top-N). APPA still on full set.\n"
...
cells.append(code(
    ...
    "\n"
    "        # #1 Hybrid re-rank: self-healing chose the candidate set + reward;\n"
    "        # raw-E5 cosine to the question decides rank order. Then page-aware\n"
    "        # context (top page(s) full text) or legacy top-N. APPA below still\n"
    "        # uses the full retrieved set.\n"
