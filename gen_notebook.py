...
cells.append(code(
    ...
    "# Generator context cap: the LLM sees only the top-N highest-scoring nodes\n"
    "# (BOTH systems, for a fair comparison). Self-healing still expands the\n"
    "# retrieved set for the reward/refusal decision -- only what the LLM reads\n"
    "# is capped, so a bigger/noisier graph context can't dilute generation.\n"
    "GEN_TOP_N = 8\n"
    "\n"
    "import sys, pathlib\n"
...
cells.append(code(
    ...
    "\n"
    "        # Generator sees only top-N (APPA below still uses the full set).\n"
    "        gen_nodes = sorted(nodes, key=lambda n: n.get('score', 0.0),\n"
    "                           reverse=True)[:GEN_TOP_N]\n"
...
cells.append(code(
    ...
    "        # raw-E5 cosine to the question decides the ORDER the LLM sees,\n"
    "        # then cap to top-N. APPA below still uses the full retrieved set.\n"
    "        gen_nodes = rerank_by_raw_e5(result['nodes'], q_vec,\n"
    "                                     doc_id, parsed)[:GEN_TOP_N]\n"
    "        prompt = build_prompt(\n"
    "            question, gen_nodes,\n"
    "            parsed or {'elements': []},\n"
