...
cells.append(code(
    ...
    "    \"\"\"Page-aware context: pick the top-`n_pages` pages (by first appearance\n"
    "    in the already-ranked node list) and return synthetic node dicts for\n"
    "    ALL elements on those pages, in reading order. build_prompt re-resolves\n"
    "    each node's text via (node_type, local_idx), so we emit those exactly.\n"
    "    DocVQA answers are one short span on one page -- giving the LLM the\n"
    "    whole confident page (not scattered cells) lets it find it.\"\"\"\n"
    ...
    "    for e in elements:\n"
    "        type_to_elems.setdefault(e['type'], []).append(e)\n"
    "    # Rank pages by first appearance among ranked nodes.\n"
    "    page_order = []\n"
