...
cells.append(code(
    ...
    "\n"
    "# Question-conditioned retrieval: embed the QUESTION with the same E5\n"
    "# model/space the nodes used (asymmetric instruction prefix), instead of\n"
    "# the doc-mean proxy which ignored the question entirely.\n"
    "_QUERY_MODEL = None\n"
    ...
    "    if _pilot_set is not None and doc_id not in _pilot_set:\n"
    "        continue\n"
    "    # Raw-E5 FAISS index (semantic). New filename so stale 256-dim\n"
    "    # _faiss.index files from the projected era are never reloaded.\n"
    "    index_path = EMB_DIR / f'{doc_id}_rawfaiss.index'\n"
...
cells.append(code(
    ...
    "\n"
    "        # VECTOR/FAISS path: question-conditioned raw-E5 query (semantic).\n"
    "        q_vec = embed_query(question)\n"
    "\n"
    "        if graph and parsed and route in ('GRAPH', 'HYBRID'):\n"
    "            # HGT path needs a query in the 256-dim node-feature space.\n"
    "            # The projection is random/unsaved, so we keep the original\n"
    "            # projected-mean proxy here (dimensionally valid; same as the\n"
    "            # pre-fix behaviour). Vector path gets the real semantic fix.\n"
    "            tp = EMB_DIR / f'{doc_id}_text.pt'\n"
    "            q_graph = (torch.load(tp, weights_only=True).mean(0)\n"
    "                       if tp.exists() else torch.zeros(256))\n"
    "            result = retriever.retrieve(q_graph, graph, parsed, [])\n"
    "        else:\n"
    "            vi    = faiss_indices.get(doc_id)\n"
